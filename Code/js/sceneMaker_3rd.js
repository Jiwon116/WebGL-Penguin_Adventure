...
function animate() {
    ...
    camera.lookAt(meshes["penguin"].position); // Camera's looking at the penguin position.
...
function penguinMovement() {
    ...
    // When move left
    if (moveLeft) {
        if(moving == -1 && penguin_box.position.x< 10){
    ...
    if (moveUp){
        if(moving == -1){
          // Force y-values of vectors by 10
